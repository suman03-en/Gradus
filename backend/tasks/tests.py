...
class BulkTaskEvaluationTests(APITestCase):
    def setUp(self):
        # Setup Teacher
        self.teacher = User.objects.create_user(username="teacher1", password="password", is_student=False)
        TeacherProfile.objects.create(user=self.teacher, department="CT", designation="Asst. Prof")
        
        # Setup Students
        self.student1 = User.objects.create_user(username="student1", password="password", is_student=True)
        StudentProfile.objects.create(user=self.student1, roll_no="THA079BEI001", department="CT")
        
        self.student2 = User.objects.create_user(username="student2", password="password", is_student=True)
        StudentProfile.objects.create(user=self.student2, roll_no="THA079BEI002", department="CT")

        # Setup Classroom and Task
        self.classroom = Classroom.objects.create(name="Math 101", created_by=self.teacher)
    ...
    def test_bulk_evaluation_wrong_mode(self):
        ...
        self.assertEqual(response.data['error'], "Bulk upload is only for offline tasks.")
