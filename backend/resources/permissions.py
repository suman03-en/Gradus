...
class HasResourcePermission(permissions.BasePermission):
    ...
    def _check_object_permission(self, request, target_obj, action):
        ...
        if isinstance(target_obj, Classroom):
            is_creator = target_obj.created_by == user
            is_student = target_obj.students.filter(id=user.id).exists()
        elif isinstance(target_obj, Task):
            is_creator = target_obj.created_by == user or target_obj.classroom.created_by == user
            is_student = target_obj.classroom.students.filter(id=user.id).exists()
        else:
            return False
