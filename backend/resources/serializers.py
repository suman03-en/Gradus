...
class ResourceSerializer(serializers.ModelSerializer):
    content_type = serializers.ChoiceField(choices=[('classroom', 'Classroom'), ('task', 'Task')], write_only=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if request and hasattr(request, 'user') and request.user.is_authenticated:
            ...
            # Fetch all Classrooms and Tasks created by this teacher
            for c in Classroom.objects.filter(created_by=user):
                choices.append((str(c.id), f"Classroom: {c.name}"))
            
            for t in Task.objects.filter(created_by=user):
                choices.append((str(t.id), f"Task: {t.name}"))
                
            if choices:
                self.fields['object_id'] = serializers.ChoiceField(choices=choices, write_only=True)

    class Meta:
        model = Resource
        fields = ['id', 'name', 'file', 'content_type', 'object_id', 'uploaded_by', 'uploaded_at', 'target_type', 'target_id']
    ...
    def validate(self, attrs):
        ...
        if not self.instance:
            ...
            if ct_model not in ['classroom', 'task']:
                raise serializers.ValidationError({"content_type": "Must be 'classroom' or 'task'."})
